...
#define ROW_IS_MARKED_DEAD(row_mark) (row_mark < ALIVE_EIGEN)
#define ROW_IS_ALIVE(r) (Row[r].shared2.mark >= ALIVE_EIGEN)
#define COL_IS_DEAD(c) (Col[c].start < ALIVE_EIGEN)
#define COL_IS_ALIVE(c) (Col[c].start >= ALIVE_EIGEN)
...
struct colamd_col {
  ...
  union {
    ...
    /* col, if the column is ALIVE_EIGEN */
    IndexType parent; /* parent in parent tree super-column structure, if */
    /* the column is dead */
  } shared1;
  union {
    IndexType score; /* the score used to maintain heap, if col is ALIVE_EIGEN */
...
static IndexType find_ordering(IndexType n_row, IndexType n_col, IndexType Alen,
                               ...
                               IndexType A[], IndexType head[], IndexType n_col2, IndexType max_deg,
                               IndexType pfree);
...
template <typename IndexType>
static IndexType garbage_collection(IndexType n_row, IndexType n_col, Colamd_Row<IndexType> Row[],
...
static bool colamd(IndexType n_row, IndexType n_col, IndexType Alen, IndexType *A, IndexType *p,
  ...
  ngarbage =
      Eigen::internal::find_ordering(n_row, n_col, Alen, Row, Col, A, p, n_col2, max_deg, 2 * nnz);
...
static void init_scoring(
  ...
  dense_row_count =
      numext::maxi(IndexType(0), numext::mini(IndexType(knobs[COLAMD_DENSE_ROW] * n_col), n_col));
  dense_col_count =
  ...
  for (k = 0; k < n_col2; /* 'k' is incremented below */) {
    ...
    while (cp < cp_end) {
      ...
      while (rp < rp_end) {
        /* get a column */
        col = *rp++;
        /* add the column, if ALIVE_EIGEN and untagged */
