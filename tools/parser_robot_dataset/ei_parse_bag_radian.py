"""Example: Register types from idl files."""
import math 
from pathlib import Path
import os,json
from cv_bridge import CvBridge
import cv2
from rosbags.typesys import Stores, get_types_from_msg, get_typestore
from rosbags.highlevel import AnyReader
from rosbags.rosbag2 import Reader

class parserManager:
    def __init__(self,bag_path):
        self.bag_path = bag_path

    def guess_msgtype(self,path: Path) -> str:
        """Guess message type name from path."""
        name = path.relative_to(path.parents[2]).with_suffix('')
        if 'idl' not in name.parts:
            name = name.parent / 'idl' / name.name
        return str(name)
	
    def rejister_msg(self,):
        typestore = get_typestore(Stores.ROS2_HUMBLE)
        add_types = {}
        ##  motor_msgs/idl/JointStateArray
        for pathstr in ['/opt/motor_msgs/idl/JointState.idl', '/opt/motor_msgs/idl/JointStateArray.idl',
                        '/opt/caicrobot_msgs/idl/JointState.idl', '/opt/caicrobot_msgs/idl/JointStateArray.idl',
                        '/opt/caicrobot_msgs/idl/JointCommand.idl','/opt/caicrobot_msgs/idl/JointCommandArray.idl']:
            msgpath = Path(pathstr)
            msgdef = msgpath.read_text(encoding='utf-8')
            add_types.update(get_types_from_msg(msgdef,self.guess_msgtype(msgpath)))
        typestore.register(add_types)
        return typestore

    def img_topic_type(self,):
        img_topic = ['/image_raw','/image_raw/compressed','/image_depth']  
        img_format =['bgr8','bgr8','16UC1']  
        img_topic_map = dict(zip(img_topic,img_format))  
        return img_topic_map
    
    def parser_join(self,reader,bag_path_file,topic,parser_sub_dir_nm="parser_robot_dataset"):
        ### topic  /joint_command_R485  /joint_state_R485 /joint_state
        out_dir_nm = topic.replace("/","")
        out_joint_dir =  os.path.join(os.path.dirname(bag_path_file), parser_sub_dir_nm,os.path.basename(bag_path_file),out_dir_nm)  
        if not os.path.exists(out_joint_dir):
            os.makedirs(out_joint_dir)  
        connections = [x for x in reader.connections if x.topic == topic]
        all_joint_info =[]
        print("start parse joint ...")
        for connection, timestamp, rawdata in reader.messages(connections=connections):
            msg = reader.deserialize(rawdata, connection.msgtype)
            #print(idl.header.frame_id,idl.header.stamp.sec,idl.header.stamp.nanosec)
            message_nan_time = str(msg.header.stamp.sec) + "."+ str(msg.header.stamp.nanosec).rjust(9, '0')
            #message_time = "{0}.{1}".format( str(timestamp)[0:10],str(timestamp)[10:] )
            message_time = message_nan_time
            #print("idl.states len",len(idl.states))
            status_info ={"message_time":message_time}
            joint_info ={}
            for sig_status in msg.states:
                 #joint_info[sig_status.name] =( sig_status.position,sig_status.velocity,sig_status.effort)
                 joint_info[sig_status.name] =( sig_status.position)
            status_info[message_time] =  joint_info
            all_joint_info.append(status_info)
        join_file_out=os.path.join(out_joint_dir,"joint.json")
        print('parse joint state end,共提取joint 消息个数: %d' % len(all_joint_info),join_file_out)
        with open(join_file_out, 'w') as f:
            json.dump(all_joint_info, f, indent=2) 

    def parser_hand_state_join(self,reader,bag_path_file,topic,parser_sub_dir_nm="parser_robot_dataset"):
        out_dir_nm = topic.replace("/","")
        out_joint_dir =  os.path.join(os.path.dirname(bag_path_file), parser_sub_dir_nm,os.path.basename(bag_path_file),out_dir_nm)  
        if not os.path.exists(out_joint_dir):
            os.makedirs(out_joint_dir)  
        connections = [x for x in reader.connections if x.topic == topic]
        all_joint_info =[]
        print("start parse joint ...")
        for connection, timestamp, rawdata in reader.messages(connections=connections):
            msg = reader.deserialize(rawdata, connection.msgtype)
            #print(idl.header.frame_id,idl.header.stamp.sec,idl.header.stamp.nanosec)
            message_nan_time = str(msg.header.stamp.sec) + "."+ str(msg.header.stamp.nanosec).rjust(9, '0')
            #message_time = "{0}.{1}".format( str(timestamp)[0:10],str(timestamp)[10:] )
            message_time = message_nan_time
            #print("idl.states len",len(idl.states))
            status_info ={"message_time":message_time}
            joint_info ={}
            for sig_status in msg.states:
                 #joint_info[sig_status.name] =( sig_status.position,sig_status.velocity,sig_status.effort)
                 joint_info[sig_status.name] =( sig_status.position/180*math.pi)
            status_info[message_time] =  joint_info
            all_joint_info.append(status_info)
        join_file_out=os.path.join(out_joint_dir,"joint.json")
        print('parse joint state end,共提取joint 消息个数: %d' % len(all_joint_info),join_file_out)
        with open(join_file_out, 'w') as f:
            json.dump(all_joint_info, f, indent=2) 

    def parser_command_join(self,reader,bag_path_file,topic,parser_sub_dir_nm="parser_robot_dataset"):
        out_dir_nm = topic.replace("/","")
        out_joint_dir =  os.path.join(os.path.dirname(bag_path_file), parser_sub_dir_nm,os.path.basename(bag_path_file),out_dir_nm)  
        if not os.path.exists(out_joint_dir):
            os.makedirs(out_joint_dir)  
        connections = [x for x in reader.connections if x.topic == topic]
        all_joint_info =[]
        print("start parse joint action ...")
        for connection, timestamp, rawdata in reader.messages(connections=connections):
            msg = reader.deserialize(rawdata, connection.msgtype)
            message_nan_time = str(msg.header.stamp.sec) + "."+ str(msg.header.stamp.nanosec).rjust(9, '0')
            message_time = message_nan_time
            status_info ={"message_time":message_time}
            joint_info ={}
            for sig_status in msg.commands:
                # joint_info[sig_status.name] =( sig_status.position,sig_status.velocity,sig_status.effort,
                # sig_status.kp ,sig_status.kd,sig_status.control_mode)
                 joint_info[sig_status.name] =( sig_status.position)
            status_info[message_time] =  joint_info
            all_joint_info.append(status_info)
        join_file_out=os.path.join(out_joint_dir,"joint.json")
        print('parse joint end,共提取joint 消息个数: %d' % len(all_joint_info),join_file_out)
        with open(join_file_out, 'w') as f:
            json.dump(all_joint_info, f, indent=2) 

    def parser_hand_command_join(self,reader,bag_path_file,topic,parser_sub_dir_nm="parser_robot_dataset"):
        out_dir_nm = topic.replace("/","")
        out_joint_dir =  os.path.join(os.path.dirname(bag_path_file), parser_sub_dir_nm,os.path.basename(bag_path_file),out_dir_nm)  
        if not os.path.exists(out_joint_dir):
            os.makedirs(out_joint_dir)  
        connections = [x for x in reader.connections if x.topic == topic]
        all_joint_info =[]
        print("start parse joint action ...")
        for connection, timestamp, rawdata in reader.messages(connections=connections):
            msg = reader.deserialize(rawdata, connection.msgtype)
            message_nan_time = str(msg.header.stamp.sec) + "."+ str(msg.header.stamp.nanosec).rjust(9, '0')
            message_time = message_nan_time
            status_info ={"message_time":message_time}
            joint_info ={}
            for sig_status in msg.commands:
                # joint_info[sig_status.name] =( sig_status.position,sig_status.velocity,sig_status.effort,
                # sig_status.kp ,sig_status.kd,sig_status.control_mode)
                 joint_info[sig_status.name] =( sig_status.position/180*math.pi)
            status_info[message_time] =  joint_info
            all_joint_info.append(status_info)
        join_file_out=os.path.join(out_joint_dir,"joint.json")
        print('parse joint end,共提取joint 消息个数: %d' % len(all_joint_info),join_file_out)
        with open(join_file_out, 'w') as f:
            json.dump(all_joint_info, f, indent=2) 


    def parse_img(self,reader,bag_path_file,img_topic,parser_sub_dir_nm="parser_robot_dataset"):
        topics_map = self.img_topic_type()
        if img_topic in topics_map.keys():
            print("img_topic :",img_topic)
            out_topic_nm = img_topic.replace("/","")
            img_connections = [x for x in reader.connections if x.topic == img_topic]
            index_img = 0
            bridge = CvBridge()
            out_img_dir =  os.path.join(os.path.dirname(bag_path_file), parser_sub_dir_nm,os.path.basename(bag_path_file),out_topic_nm)
            if not os.path.exists(out_img_dir):
                os.makedirs(out_img_dir)
            print("start parse img ...")
            for connection, timestamp, rawdata in reader.messages(connections=img_connections):
                msg = reader.deserialize(rawdata, connection.msgtype)
                if  img_topic=='/image_raw/compressed' :
                    cv_image = bridge.compressed_imgmsg_to_cv2(msg,topics_map[img_topic])
                else :
                    cv_image = bridge.imgmsg_to_cv2(msg,topics_map[img_topic])
                out_img_nm = os.path.join(out_img_dir,  "{0}.{1}.jpg".format(str(timestamp)[0:10],str(timestamp)[10:].rjust(9, '0')))
                cv2.imwrite(out_img_nm, cv_image)
                index_img += 1
            print('parser_robot_dataset image end,共提取图像个数: %d' % index_img,out_img_dir)
        else:
            print("not match img topic  {}".format(topics_map))

    def parser_rosbag2_all(self,sub_dir_nm="parse"):
        custorm_typestore = self.rejister_msg()
        bag_path_file = self.bag_path
        bagpath = Path(bag_path_file)
        
        # Create reader instance and open for reading.
        message_topics = []
        with AnyReader([bagpath], default_typestore=custorm_typestore) as reader:
            for connection in reader.connections:
                message_topics.append(connection.topic)
                print(connection.topic, connection.msgtype)  
            if "/hand_state" in message_topics:
                self.parser_hand_state_join(reader,bag_path_file,"/hand_state",parser_sub_dir_nm=sub_dir_nm)
            if "/joint_state" in message_topics:
                self.parser_join(reader,bag_path_file,"/joint_state",parser_sub_dir_nm=sub_dir_nm)    
            if "/hand_command" in message_topics:
                self.parser_hand_command_join(reader,bag_path_file,"/hand_command",parser_sub_dir_nm=sub_dir_nm)
            if "/joint_command_upper_body" in message_topics:
                self.parser_command_join(reader,bag_path_file,"/joint_command_upper_body",parser_sub_dir_nm=sub_dir_nm)
            if "/joint_command" in message_topics:
                self.parser_command_join(reader,bag_path_file,"/joint_command",parser_sub_dir_nm=sub_dir_nm)    
            img_topics = ['/image_raw','/image_raw/compressed']
            for img_topic in img_topics:
                if img_topic in message_topics:
                    self.parse_img(reader,bag_path_file,img_topic,parser_sub_dir_nm=sub_dir_nm)
            print("hell")

if __name__ =="__main__":
    root_batch = r"/data/cloud/clips/temp/20251222"
    for bag_nm in  os.listdir(root_batch):
        #if  bag_nm.startswith("rosbag2_"):
        if  bag_nm.startswith("1_"):
            bag_path = os.path.join(root_batch,bag_nm)
            custome_parser = parserManager(bag_path)
            print("bag_path",bag_path)
            custome_parser.parser_rosbag2_all(sub_dir_nm="parse_degree")
 