...
def format_embodies_data(src_root_sub_dir,join_sub_nm,img_dir_name = r"image_raw"):
    ...
    i = 0
    for img_file  in img_file_list:   
        match_joint_time = find_match_joint_time(img_file,jont_times,match_gap=0.5)
...
def img_2_h264_mp4(img_dir_path,out_dataset_dir,ep_index=1,video_fps=30):
    ...
    fps = video_fps  # 帧率
    # 打开文件（如果文件不存在将会被创建）
    output_dir = os.path.join(out_dataset_dir,"videos","chunk-000","observation.images.ego_view")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    output_filename = os.path.join(output_dir,episode_video_nm)
    ffmpeg_cmd = 'cd {0} && ffmpeg   -i  {1}/%d.jpg -framerate {2}  -c:v libx264 -pix_fmt yuv420p  -r {2} -y {3} '.format(output_dir,img_dir_path,str(fps),output_filename)  
...
def json_2_parquet(parsed_json_dir,out_dataset_dir,ep_index=1,task_id=0 ):
    ...
    # ['hand_command','hand_state','joint_command','joint_state' ]
    joint_state_json = os.path.join(parsed_json_dir,"joint_state_match.json")
    hand_state_json = os.path.join(parsed_json_dir,"hand_state_match.json")
    observation_state = get_match_action_state_dat(joint_state_json,hand_state_json)
    joint_action_json = os.path.join(parsed_json_dir,"joint_command_upper_body_match.json")
    hand_action_json  = os.path.join(parsed_json_dir,"hand_command_match.json")
    observation_action = get_match_action_state_dat(joint_action_json,hand_action_json)
    #print(observation_state)
    out_parque_dir = os.path.join(out_dataset_dir,"data","chunk-000")
...
def generate_modality_json(join_state_json,hand_state_json,join_action_json,hand_action_json,dst_dataset_dir):
    out_meta_dir = os.path.join(dst_dataset_dir,"meta")
...
def process_dataset_data(root_dir):
    parsed_dir = os.path.join(root_dir,"parser_robot_dataset")
    dst_dataset_dir = os.path.join(root_dir,"dataset")
    if not os.path.exists(dst_dataset_dir):
        os.makedirs(dst_dataset_dir)
    tasks_jsonl    = os.path.join(root_dir,"tasks.jsonl")
    ...
    episodes_jsonl_file = os.path.join(dst_dataset_dir,"meta","episodes.jsonl") 
    if os.path.exists(episodes_jsonl_file):
        os.remove(episodes_jsonl_file)
    print("step 2: dataset start"    )
    for i, clip_nm in enumerate(sorted(os.listdir(parsed_dir))) :
        ...
        for sig_ontolo in join_sub_nm_info:
            format_embodies_data(parsed_clip_path,sig_ontolo)
        ...
        ### 2.3 生成meta数据
        joint_state_json_eg = os.path.join(parsed_clip_path,"joint_state_match.json")
        hand_state_json_eg = os.path.join(parsed_clip_path,"hand_state_match.json")
        joint_acton_json_eg = os.path.join(parsed_clip_path,"joint_command_upper_body_match.json")
        hand_action_json_eg = os.path.join(parsed_clip_path,"hand_command_match.json")
        ...
        print("step 2.3.2生成 episodes.jsonl"    )
...
if __name__ =="__main__":
    root_dir_exp = r"/data/cloud/ros2/rosbag2_motor/batch_01_select/" 
    process_dataset_data(root_dir_exp)
