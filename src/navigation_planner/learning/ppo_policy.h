...
struct PPOConfig {
    double learning_rate = 3e-4;     // 0.0003 as per PRD
    double gamma = 0.999;            // Discount factor as per PRD
    double lam = 0.95;               // GAE lambda as per PRD
    ...
    int batch_size = 64;
    int epochs = 10;
    int max_training_steps = 15000;   // Training steps as per PRD
    int max_episode_steps = 200;      // Max steps per episode as per PRD
...
public:
    ...
    /**
     * @brief Get max episode steps from config
     * @return Max steps per episode
     */
    int getMaxEpisodeSteps() const { return config_.max_episode_steps; }
    
    /**
     * @brief Get gamma (discount factor)
     * @return Gamma value
     */
    double getGamma() const { return config_.gamma; }
    
    /**
     * @brief Get lambda (GAE parameter)
     * @return Lambda value
     */
    double getLambda() const { return config_.lam; }
    
    /**
     * @brief Update PPO configuration from parameter map
     */
